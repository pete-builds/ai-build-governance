...
|--:|:--|:--|:--|
| 1 | Establish the need | [Statement of Need](model/02-statement-of-need.md) | Building the wrong thing, solution-first |
| 2 | Classify the consequences | [Risk Classification](model/03-classification.md) | Risk decided by reviewer mood |
| 3 | Review the design | [Design Review](model/04-design-review.md) | A design nobody read against the requirement |
| 4 | Authorize construction | [Authorization to Build](model/05-authorization-to-build.md) | Approval becoming a permanent entitlement |
| 5 | Inspect at the right moments | [Inspections](model/06-inspections.md) | Verification after the evidence is gone |
| 6 | Approve production | [Production Approval and Records](model/07-production-approval.md) | Software arriving in production by being used more, with nobody having decided |
| 7 | Keep a record | [Production Approval and Records](model/07-production-approval.md) | Nobody being able to say what is running, what it can reach, and who owns it |
| ↺ | **When it changes, re-enter** | [Alterations and Existing Systems](model/08-alterations.md) | Change that silently outruns its review |
...
| Chapter | Applies |
|:--|:--|
| [Principles](model/01-principles.md) | Always. Where a chapter conflicts with a principle, the principle wins. |
| [Roles and Authority](model/09-roles.md) | From before the Statement of Need until decommissioning |
| [Concurrent Reviews](model/10-concurrent-reviews.md) | Alongside design review, never as a queue behind it |
| [Certified Reusable Components](model/11-certified-components.md) | Both the **entry point** for anything reusing existing work and an output of finishing |
| [Delegated Authority](model/12-delegated-authority.md) | Whenever an agent acts on a human's behalf |
| [Third-Party Capabilities](model/13-third-party.md) | Whenever a build depends on something the institution does not operate |

**Start at [Certified Reusable Components](model/11-certified-components.md), not at step 1.**
...
The thirteen chapters and how they relate are in the spine above, and
[how to read this](model/00-conventions.md) explains which sentences bind you.
...
| [07 Pilot protocol](guide/07-pilot-protocol.md) | Testing this framework, and what result would disprove it |
| [08 Vendor beta connections](guide/08-vendor-beta-mcp.md) | "A vendor wants us to beta test their tool server on our gateway" |
...
| [Pilot findings](templates/pilot-findings.md) | Reporting a pilot, including the parts that went badly |
...
| [How this compares](reference/comparison.md) | NIST, Stanford, UC, and the rest, with verification status per claim and where each is stronger |
| [Machine-readable requirements and a reference gate](reference/enforcement.md) | `requirements.json`, and a CI gate that enforces one narrow thing |
